...
SqliteStorage.prototype.initialize = function(onReadyCallback) {
  ...
  // If the adapter supports setting a schema strategy, provide it
  // This allows the adapter to coordinate pre-initialization if needed
  if (this.adapter.setSchemaStrategy && typeof this.adapter.setSchemaStrategy === 'function') {
    this.adapter.setSchemaStrategy(this.schemaStrategy);
  }
...
/**
 * Add or update inventory item using the schema strategy
 */
SqliteStorage.prototype.upsertInventory = function(collection, docId, version, callback) {
  this.ensureReady();
  const dbWrapper = this._createDbWrapper();
  this.schemaStrategy.upsertInventoryItem(dbWrapper, collection, docId, version, callback);
};

/**
 * Delete inventory item using the schema strategy
 */
SqliteStorage.prototype.deleteInventory = function(collection, docId, callback) {
  this.ensureReady();
  const dbWrapper = this._createDbWrapper();
  this.schemaStrategy.deleteInventoryItem(dbWrapper, collection, docId, callback);
};

/**
 * Update inventory using the schema strategy
 * @deprecated Use upsertInventory or deleteInventory instead
 */
